...
DB_FILE = 'db.bin'
P21DEF_KEYS = 'fck fck_min fck_max div_min div_max vdddpu chip_version'
P21DEF_DFLT_VALUES = (700, 10, 1000, 2, 3, 1100, 'v1.3')
...
P21defDatabase = namedtuple("P21def_database", P21DEF_KEYS)
p21def_database = P21defDatabase._make(P21DEF_DFLT_VALUES)
...
def get_p21def_user_values():
    global p21def_database

    # Dftl values
    fck = getattr(p21def_database, 'fck')
    fck_min = getattr(p21def_database, 'fck_min')
    fck_max = getattr(p21def_database, 'fck_max')
    div_min = getattr(p21def_database, 'div_min')
    div_max = getattr(p21def_database, 'div_max')
    vdddpu = getattr(p21def_database, 'vdddpu')
    chip_version = getattr(p21def_database, 'chip_version')
    ...
    p21def_database = p21def_database._replace(**kw)
...
if args.dimm in ["P21D", "P21E", "P21F"]:
    if not args.default:
        get_p21def_user_values()

    database = p21def_database
elif args.dimm == "P21":
    if not args.default:
        get_p21_user_values()
