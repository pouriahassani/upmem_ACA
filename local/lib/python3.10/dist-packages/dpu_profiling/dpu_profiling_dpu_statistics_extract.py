...
class DpuProfiling:
	def __init__(self, dpu_path, profiling_log_path):
		self.nb_threads = NR_THREADS_DEFAULT
		self.total_occurence = 0
		self.total_occurence_per_thread = [ x for x in range(self.nb_threads) ]
		self.dict_addr_to_function = dict()
		self.dict_function_to_occurence = dict()
		self.dict_function_to_occurence_per_thread = [ dict() for x in range(self.nb_threads) ]
	...
	def print_profiling_per_thread(self):
		for th_id in range(self.nb_threads):
			print("******************")
	...
	def extract_profiling(self):
		...
		dict_occurence = [ dict() for x in range(self.nb_threads) ]
		...
		with open(self.profiling_log_path, "r") as f:
			for line in f:
				...
				if int(th_id) > max_tid:
					max_tid = int(th_id)

		# Update actual number of threads based on max thread ID
		if max_tid != 0:
			self.nb_threads = max_tid + 1
...
def print_dpu_statistics(dpu_binary_path, profiling_log_path, per_thread, call_sites):
	dpu_profiling = DpuProfiling(dpu_binary_path, profiling_log_path)
