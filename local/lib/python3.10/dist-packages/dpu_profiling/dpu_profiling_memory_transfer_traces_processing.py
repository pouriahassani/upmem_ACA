...
def trace_end():
    ...
    module_results_parser.parse_sg_xfer(aggregate_infos(sg_xfer_infos))
...
def probe_libdpu__libdpu_dpu_transfer_matrix_add_dpu_block(
        event_name, context, common_cpu, common_secs, common_nsecs,
        common_pid, common_comm, common_callchain,
        __probe_ip,
        dpu, transfer_matrix, buffer,
...
def probe_libdpu__libdpu_sg_xfer_rank_handler(
        event_name, context, common_cpu, common_secs, common_nsecs,
        common_pid, common_comm, common_callchain,
        __probe_ip,
        rank, rank_id, cb_args,
        perf_sample_dict):
    global sg_xfer_enter_secs, sg_xfer_enter_nsecs, sg_xfer_pid
    sg_xfer_enter_secs.setdefault(common_pid, {})[rank] = common_secs
    sg_xfer_enter_nsecs.setdefault(common_pid, {})[rank] = common_nsecs
    sg_xfer_pid[common_pid] = (rank)

def probe_libdpu__libdpu_sg_xfer_rank_handler_exit__return(
        event_name, context, common_cpu, common_secs, common_nsecs,
        common_pid, common_comm, common_callchain,
        __probe_func, __probe_ret_ip,
        perf_sample_dict):
    global sg_xfer_enter_secs, sg_xfer_enter_nsecs, sg_xfer_pid
    global sg_xfer_infos
    global rank_path
    global sg_xfer_pid
    global mram_pid

    rank = sg_xfer_pid[common_pid]

    # retreive information from probe_libdpu__libdpu_dpu_copy_to_mrams
    transfer_matrix = mram_pid[common_pid][0]
    path = rank_path[mram_pid[common_pid][1]]
    size = mram_pid[common_pid][2]

    start_ns = Duration.nanoseconds(
        sg_xfer_enter_secs[common_pid][rank],
        sg_xfer_enter_nsecs[common_pid][rank])
    end_ns = Duration.nanoseconds(common_secs, common_nsecs)

    nb_dpus = len(xfer_size[transfer_matrix])
    size *= nb_dpus
    sg_xfer_infos.setdefault(common_pid, []).append(
        TransferInfo(
            start_ns,
            end_ns,
            size,
            path,
            "sg_xfer_thread_{}dpus".format(nb_dpus)))
